...
async def select_files(request):
    try:
        ...
        # Use the improved file dialog function
        def open_file_dialog():
            try:
                ...
                if threading.current_thread() is threading.main_thread():
                    # If we're already in the main thread, just run the dialog
                    return run_dialog()
                else:
                    # If we're not in the main thread, we need to use a different approach
                    # We'll use a queue to get the result back from the main thread
                    import queue
                    result_queue = queue.Queue()

                    # Define a function to run in the main thread and put the result in the queue
                    def main_thread_dialog():
                        try:
                            result = run_dialog()
                    ...
                    # We'll use the PromptServer's loop to schedule it
                    server.PromptServer.instance.loop.call_soon_threadsafe(main_thread_dialog)
                    ...
                    try:
                        ...
                        print("Timeout waiting for file dialog")
                        return []
            except Exception as e:
        ...
        # Since we're in an async context, we need to run this in an executor
