...
class AyonNode:
    ...
    def get_template_output_path(self, product_type, variant):
        ...
        try:
            ...
            if not root_path:
                # Check if we have project paths defined
                project_paths_str = ayon_env.get("AYON_PROJECT_PATHS", "{}")
                try:
                    project_paths = json.loads(project_paths_str)
                    if project_name in project_paths:
                        root_path = project_paths[project_name]
                except Exception as e:
                    log.warning(f"Could not parse AYON_PROJECT_PATHS: {e}")
            
            # If still not found, check if we have a direct path for this project
            if not root_path:
                project_root_var = f"AYON_{project_name.upper()}_ROOT"
                root_path = environ.get(project_root_var, "")
            
            # If still not found, use W: drive as a fallback for artist_playground
            if not root_path and project_name == "artist_playground":
                root_path = "W:"
            
            # Last resort fallback to ComfyUI output directory
            if not root_path:
                root_path = folder_paths.get_output_directory()
                log.warning(f"No AYON project root found, using ComfyUI output directory: {root_path}")
...
class PublishImage(AyonNode):
    ...
    def INPUT_TYPES(cls):
            ...
            try:
                ...
                if project_name:
                    ...
                    if folders:
                        ...
                        folder_options.sort()
                        log.info(f"Found {len(folder_options)} folders in project {project_name}")
                    
                    # Get tasks for the default folder
                    if default_folder_path:
                        try:
                            tasks = cls.get_tasks_for_folder(project_name, default_folder_path)
                            if tasks:
                                task_options = tasks
                                log.info(f"Found {len(task_options)} tasks for folder {default_folder_path}")
                        except Exception as e:
                            log.warning(f"Could not fetch tasks for folder {default_folder_path}: {e}")
            except Exception as e:
        ...
        return {
            "required": {
                ...
                "folder_path": (
                    "COMBO",
                    {"options": folder_options, "default": default_folder_path, "multi_select": False}
                ),
                "task_name": (
                    "COMBO",
                    {"options": task_options, "default": default_task_name, "multi_select": False}
                ),
                "variant": (
                    "COMBO",
    ...
    def main(self, image, folder_path, task_name, variant, product_type, 
            output_path="", prompt=None, extra_pnginfo=None):
    ...
    @staticmethod
    def publish_image_api(node_id, workflow, folder_path=None, task_name=None, variant=None, product_type=None):
        ...
        try:
            ...
            if not node_data:
                return {"success": False, "error": f"Node with ID {node_id} not found in workflow"}
            ...
            if widget_values and len(widget_values) >= 6:
                ...
                product_type = widget_values[3] if product_type is None else product_type
            ...
            if not output_path:
                output_path = publish_instance.get_template_output_path(product_type, variant)
            ...
            try:
                profiles = publish_instance.core_settings["tools"]["creator"]["product_name_profiles"]
                tmpl = StringTemplate(profiles[0]["template"])
            ...
            workflow_path = os.path.join(output_path, f"{filename_prefix}_{counter:05}_workflow.json")
            ...
            if not png_files:
                return {"success": False, "error": "No images found in ComfyUI output directory"}
            ...
            try:
                ...
                try:
                    ...
                    available_functions = [func for func in dir(ayon_api) if not func.startswith('_')]
                    log.info(f"Available functions in ayon_api: {available_functions}")
                    
                    # Look for functions related to versions or representations
                    version_functions = [func for func in available_functions if 'version' in func.lower()]
                    representation_functions = [func for func in available_functions if 'representation' in func.lower()]
                    
                    log.info(f"Version-related functions: {version_functions}")
                    log.info(f"Representation-related functions: {representation_functions}")
...
async def api_update_output_path(request):
    try:
        ...
        log.info(f"Updating output path for: folder={folder_path}, task={task_name}, variant={variant}, product_type={product_type}")
        ...
        try:
            ...
            if not output_path:
                output_path = os.path.join(folder_paths.get_output_directory(), "ayon_publish", product_type, variant)
...
async def api_get_tasks_for_folder(request):
    try:
        ...
        if not project_name or not folder_path:
            return web.json_response({
                "success": False,
                "error": "Missing project_name or folder_path"
            })
